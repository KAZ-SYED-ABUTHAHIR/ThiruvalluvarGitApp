...
iyal = 0,
athikaram = 0
let paalkal = details.section.detail
let iyalkal = paalkal[paal].chapterGroup.detail
let athikarankal = iyalkal[iyal].chapters.detail
let seyyulkalStart = athikarankal[athikaram].start

// let paalName = paalkal[paal].name
// let iyalName = iyalkal[iyal].name
// let athikaramName = athikarankal[athikaram].name

// c(`${paalName}, ${iyalName}, ${athikaramName}`)

let parts = []
let chapters = []
let chapterGroups = []

paalkal.forEach(paal => {
  parts.push(
    {
      name: paal.name,
      start: paal.chapterGroup.detail[0].chapters.detail[0].start,
      end: paal.chapterGroup.detail[paal.chapterGroup.detail.length-1].chapters.detail[paal.chapterGroup.detail[paal.chapterGroup.detail.length-1].chapters.detail.length-1].end,

    }
  )
  paal.chapterGroup.detail.forEach(iyal => {
    chapterGroups.push(
      {
        name: iyal.name,
        start: iyal.chapters.detail[0].start,
        end: iyal.chapters.detail[iyal.chapters.detail.length-1].end,
      }
    )
    iyal.chapters.detail.forEach(chapter =>{
      chapters.push(
        {
          name: chapter.name,
          start: chapter.start,
...
c(chapters)
// c(chapters)
// iyalkal.forEach(el => c(el.name))
// athikarankal.forEach(el => c(el.name))
